...
%PROXIMITYSIM Proximity Simulator for HawkEye KPP5
%   [ outStruct ] = proximitySim(in, j1,j2 )
%   in is a struct of output generated by fimport.m
...
%% Set Constants
    ...
    try
        if j1 ~= j2 
            % Add a little validation, in case a loop is used outside of the function
            data1 = in(j1);
            data2 = in(j2);
            ...
            for i = 1:minlen %loop through until the shortest duration
                %%
                ...
                try %isolate one row of PV values
                ...
                %% Add PVT evaluations to the stack, again
                xkj2stack(i,:) = xkj2t;
                vkj2stack(i,:) = vkj2t;
                rkj2stack(i,:) = rkj2t;        

                %% Perform position conversions
                % Add the quantization and bit crunching to a struct
                ...
                %% Evaluate the link budget for the location and for that point in time relative to the other jumper
               ...
               %% Safe Zone Check 
               % Here's the rather anti-climactic safe zone check. Only 
               % need one check. If this were real, we would also need to
               % track all the assets that could possibly enter the safe 
               % zone in the near future, and be ready to evaluate the 
               % distances to each of them.
               %% Run the safe zone check
               if fD(i)<safeRadius
            ...
            if plotflag  
                ...
                %% Deal with floating point math error
        ...
        %% Catch an interrupt, but still exit clean.
        % The end of the plot can run long, so heres a way to catch the Ctrl-C 
